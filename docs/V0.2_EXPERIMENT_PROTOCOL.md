...
**Status:** superseded as the *next implementation*  
**Superseded by:** [V0.2 regime protocol](V0.2_REGIME_PROTOCOL.md)  
**Deferred policy work:** this file is now the **v0.3** candidate (H_unknown, DEFER, receding-horizon policy). Do not implement it until the regime experiment answers whether \(\Delta_{loop}(Q)>0\) anywhere.

The v0.1.1 inference loop is frozen. Adding a new decision policy before a hard, matched-backend experiment would confound the result.

---

# QUASAR2 v0.3 (deferred) — original v0.2 policy protocol

**Status:** frozen pre-implementation specification, **not opened**  
**Protocol version:** 0.3.0-deferred  
...
QUASAR2 v0.3 (formerly labeled v0.2). It is a protocol, not a statement that the proposed policy is
superior. Implementation begins only after the v0.2 regime experiment is complete.
