...
def build_repository_state_manifest(root: str | Path) -> dict[str, Any]:
    ...
    capabilities = [
        ...
        _capability(
            capability_id="recoverability_empirical_voi_bench",
            claimed_status="synthetic recoverability vs VoI implemented",
            verified_status=UNTESTED,
            implementation_files=("src/quasar2/eval/recoverability_bench.py", "src/quasar2/recoverability.py"),
            test_files=("tests/test_v2_recoverability.py",),
            cli_or_api="quasar2 recoverability-bench",
            artifact_paths=("experiments/runs/",),
            last_verified_commit=commit,
            validation_command="python -m unittest tests.test_v2_recoverability",
            notes="Gate 1 is not claimed. Learned estimator is synthetic-only.",
        ),
        _capability(
            capability_id="shadow_study_sanity",
            claimed_status="shadow transition matrix CLI present",
            verified_status=UNTESTED,
            implementation_files=("src/quasar2/eval/shadow_study.py",),
            test_files=("tests/test_v2_legacy_compat.py",),
            cli_or_api="quasar2 shadow-study --register",
            artifact_paths=("experiments/runs/",),
            last_verified_commit=commit,
            validation_command="quasar2 shadow-study --limit 2 --conditions q0 --output experiments/runs --run-id shadow-smoke --overwrite --register",
            notes="Does not overwrite experiments/results/frozen or benchmark.json.",
        ),
        _capability(
            capability_id="synthetic_oracle_policy_track",
            claimed_status="tabular oracle + learned imitation on synthetic states",
            verified_status=UNTESTED,
            implementation_files=("src/quasar2/eval/oracle_env.py", "src/quasar2/decision/policies.py"),
            test_files=("tests/test_v2_recoverability.py",),
            cli_or_api="quasar2 policy-compare",
            artifact_paths=("experiments/runs/",),
            last_verified_commit=commit,
            validation_command="python -m unittest tests.test_v2_recoverability.OraclePolicyTests",
            notes="Must not be run on WDI with proxy kernels claimed as oracle.",
        ),
    ]
    return {
