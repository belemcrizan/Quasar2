...
- Engineering hardening: indexed BM25, corrected signed hashing, content/revision-aware atomic neural caches, strict HTTP and budget contracts, recursive runtime gold isolation, finite numeric/bootstrap validation, finite-sample conformal quantiles, and complete-tuple WDI evaluation. Adds 54 regression tests and CI. See [validation and compatibility notes](docs/engineering-hardening.md); hybrid sanity accuracy decreases by one query, and frozen historical results remain unchanged.
